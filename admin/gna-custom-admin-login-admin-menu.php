...
if (!class_exists('GNA_Custom_AdminLogin_Admin_Menu')) {
	abstract class GNA_Custom_AdminLogin_Admin_Menu {
		/**
		...
		function show_msg_settings_updated() {
			...
			_e('Settings successfully updated.', 'gna-custom-admin-login');
			echo '</strong></p></div>';
		}

		static function show_msg_record_deleted_st() {
			echo '<div id="message" class="updated fade"><p><strong>';
			_e('The selected record(s) deleted successfully!', 'gna-custom-admin-login');
			echo '</strong></p></div>';
		}
